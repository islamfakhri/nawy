(() => {
  ...
  /* =====================================================
     NAWY APP — v5
     الحفاظ على اسم الدالة addTask كما هو
  ===================================================== */
  ...
  function loadTasks() {
    const current = parseTasks(STORAGE_KEY);
    if (current) return current;

    for (const key of LEGACY_KEYS) {
      const legacy = parseTasks(key);
      if (legacy) {
        try {
  ...
  function updateSoundButton() {
    ...
    soundToggle.setAttribute("aria-label", enabled ? "إيقاف الأصوات" : "تشغيل الأصوات");
  }
  ...
  function ensureAudioContext() {
    ...
    try {
      ...
      console.warn("Nawy audio error:", error);
      return null;
    }
  }

  function playTone({ type = "sine", startFrequency, endFrequency = startFrequency, volume = 0.05, duration = 0.1 }) {
    ...
    try {
      ...
      if (endFrequency !== startFrequency) {
        osc.frequency.exponentialRampToValueAtTime(endFrequency, now + duration * .7);
      }
      ...
      console.warn("Nawy tone error:", error);
    }
  }

  function playAddSound() {
    playTone({ type: "sine", startFrequency: 430, endFrequency: 680, volume: 0.048, duration: 0.12 });
  }

  function playCompletionSound() {
    playTone({ type: "triangle", startFrequency: 1046.5, volume: 0.055, duration: 0.22 });
    window.setTimeout(() => playTone({ type: "triangle", startFrequency: 1318.5, volume: 0.04, duration: 0.2 }), 65);
  }
  ...
  function formatTaskDate(iso) {
    if (!iso) return "";
    try {
      return new Intl.DateTimeFormat("ar-EG", { hour: "numeric", minute: "2-digit" }).format(new Date(iso));
    } catch {
      return "";
    }
  }
  ...
  function updateProgress() {
    ...
    const done = tasks.reduce((sum, task) => sum + (task.completed ? 1 : 0), 0);

    if (total === 0) {
      progressFill.style.width = "0%";
      progressText.textContent = "ابدأ أول نية";
      return;
    }
    ...
    if (percentage === 100) progressText.textContent = "ما شاء الله! كملت يومك 🎉";
    else if (done === 0) progressText.textContent = `${total} نية اليوم`;
  ...
  function render() {
    ...
    const activeTasks = tasks.filter((task) => !task.completed).sort(sortActive);
    const completedTasks = tasks.filter((task) => task.completed).sort(sortCompleted);
    ...
    if (tasks.length === 0) {
      ...
      empty.innerHTML = `
        ...
        <strong>لسه مفيش نوايا هنا</strong>
        <span>اكتب أول حاجة ناوي تعملها وابدأ يومك.</span>
      `;
      tasksContainer.appendChild(empty);
      return;
    }

    activeTasks.forEach((task) => tasksContainer.appendChild(createTaskElement(task)));

    if (completedTasks.length > 0) {
      const divider = document.createElement("div");
      divider.className = "completed-divider";
      const span = document.createElement("span");
      span.textContent = `النوايا المحققة (${completedTasks.length})`;
  ...
  function createTaskElement(task) {
    ...
    check.setAttribute("aria-label", task.completed ? "إلغاء إنجاز النية" : "تحديد النية كمكتملة");
    ...
    if (task.completedAt) {
      ...
      meta.textContent = `تحققت ${formatTaskDate(task.completedAt)}`;
    ...
    edit.setAttribute("aria-label", "تعديل النية");
    edit.textContent = "✎";
    ...
    remove.setAttribute("aria-label", "حذف النية");
    remove.textContent = "×";
  ...
  function showCompletionFeedback(id) {
    ...
    const taskElement = tasksContainer.querySelector(`[data-task-id="${CSS.escape(id)}"]`);
    if (!taskElement) return;

    taskElement.classList.add("just-completed");
    const check = taskElement.querySelector(".check");
    if (!check) return;
    ...
    for (let i = 0; i < 12; i++) {
      ...
      dot.style.setProperty("--distance", `${29 + (i % 3) * 7}px`);
      dot.style.setProperty("--size", `${4 + (i % 2)}px`);
  ...
  function updateSubmitState() {
    submitBtn.classList.toggle("active", taskInput.value.trim().length > 0);
  }
  ...
  toastUndo.addEventListener("click", restoreDeletedTask);
  ...
  function closeInstallOverlay(remember = true) {
    installOverlay.hidden = true;
    if (remember) localStorage.setItem(INSTALL_DISMISSED_KEY, "1");
  }
  ...
  function showManualInstallGuide() {
    ...
    guideText.textContent = shareText;
  ...
  window.addEventListener("beforeinstallprompt", (event) => {
    ...
    // لا نزعج المستخدم من أول ثانية؛ أظهرها بعد أول تفاعل أو بعد فترة قصيرة.
    window.setTimeout(() => {
      if (!isStandalone() && !document.hidden) showInstallOverlay();
    }, 9000);
  });
  ...
  installClose.addEventListener("click", () => closeInstallOverlay(true));
  guideClose.addEventListener("click", () => { installGuide.hidden = true; });
  installGuide.addEventListener("click", (event) => {
    if (event.target === installGuide) installGuide.hidden = true;
  });

  /* بعد إضافة أول نية، يمكن تذكير المستخدم بالتثبيت فقط إذا لم يكن مثبتًا */
  let installHintedAfterTask = false;
  function maybeSuggestInstallAfterTask() {
    if (installHintedAfterTask || isStandalone() || !deferredInstallPrompt || !shouldShowInstallPrompt()) return;
    installHintedAfterTask = true;
    window.setTimeout(showInstallOverlay, 1400);
  }

  const originalAddTask = addTask;
  addTask = function patchedAddTask(text) {
    const result = originalAddTask(text);
    if (result) maybeSuggestInstallAfterTask();
    return result;
  };
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", async () => {
      try {
        ...
        console.warn("Nawy Service Worker error:", error);
      }
    });
  }
  ...
  // Desktop فقط: تركيز سريع على خانة النية.
  if (window.innerWidth >= 760 && !isStandalone()) {
    window.setTimeout(() => taskInput.focus(), 250);
  }

  // اختصار لوحة مفاتيح: / يركز على الإدخال.
  window.addEventListener("keydown", (event) => {
    if (event.key === "/" && document.activeElement !== taskInput && document.activeElement !== editInput) {
      event.preventDefault();
      taskInput.focus();
    }
    if (event.key === "Escape") {
      if (!editModal.hidden) closeEdit();
      if (!installGuide.hidden) installGuide.hidden = true;
      if (!installOverlay.hidden) closeInstallOverlay(false);
    }
  });
})();
